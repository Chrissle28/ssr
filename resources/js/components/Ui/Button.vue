<template>
    <component
        ...
        :class="{
            ...
            'text-base': size_ == 'md',
            'text-xs': size_ == 'sm',
            'rounded-lg': !square && size_ == 'lg' && !text,
            'rounded-md': !square && size_ == 'md' && !text,
            'rounded-sm': !square && size_ == 'sm' && !text,
            'px-6 py-3': size_ == 'lg' && !text,
            'px-5 py-2': size_ == 'md' && !text,
            'px-4 py-1': size_ == 'sm' && !text,
            ...
            'bg-white hover:bg-yellow-100 active:bg-yellow-300 text-yellow hover:text-yellow-600 active:text-yellow-700 border border-yellow focus:ring-yellow-400':
                variant_ == 'yellow' && outline && !text && !disabled,
            'bg-black hover:bg-gray-900 active:bg-gray-900 text-white active:text-white border border-black focus:ring-gray-900':
                variant_ == 'black' && !outline && !text && !disabled,
            'bg-transparent hover:bg-black active:bg-gray-900 text-black hover:text-white active:text-white border border-black focus:ring-gray-800':
                variant_ == 'black' && outline && !text && !disabled,
...
import { getSize, getVariant, variants, sizes } from '../../app.config';
...
export default defineComponent({
    ...
    setup(props, { attrs }) {
        const tag = 'href' in attrs ? Link : 'button';
